    ...
    internal class TabGeneralCtrl
    {
        ...
        void HandleFileDrop(DragEventArgs args)
        {
            ...
            foreach (var filename in filenames)
            {
                ...
                if (names.Contains(scriptName) && !Apis.Misc.UI.Confirm($"{I18N.ReplaceScript} {scriptName}"))
                {
                    continue;
                }
        ...
        private void BindControlsEvent(Services.LuaServer luaServer)
        {
            btnDelAll.Click += (s, a) =>
            {
                if (!Apis.Misc.UI.Confirm(I18N.ConfirmDeleteAllScripts))
                {
                    return;
                }
            ...
            btnImport.Click += (s, a) =>
            {
                try
                {
                    var serializedScripts = Apis.Misc.UI.ReadFileContentFromDialog(
            ...
            btnExport.Click += (s, a) =>
            {
                try
                {
                    ...
                    Apis.Misc.UI.SaveToFile(Apis.Models.Consts.Files.TxtExt, serializedScripts);
                }
                catch
                {
        ...
        void RefreshFlyPanel()
        {
           ...
           {
               var ctrls = luaServer.GetAllLuaCoreCtrls();
               var removed = Apis.Misc.UI.DoHouseKeeping<LuaUI>(flyLuaUiPanel, ctrls.Count);
