...
namespace Luna.Controllers.FormEditorCtrl
{
    internal sealed class MenuCtrl
    {
        ...
        {
            Apis.Misc.UI.CloseFormIgnoreError(formEditor);
        }

        void OnMiLoadClickHandler(object sender, EventArgs args)
        {
            if (editorCtrl.IsChanged()
                    && !Apis.Misc.UI.Confirm(I18N.DiscardUnsavedChanges))
            {
                return;
            }

            var cf = Apis.Misc.UI.ReadFileFromDialog(Apis.Models.Consts.Files.LuaExt);
            var script = cf.Item1;
        ...
        void OnMiSaveAslickHandler(object sender, EventArgs args)
        {
            ...
            var err = Apis.Misc.UI.ShowSaveFileDialog(
                Apis.Models.Consts.Files.LuaExt,
                script,
                out var filename);
            ...
            switch (err)
            {
                ...
                case Apis.Models.Datas.Enums.SaveFileErrorCode.Fail:
                    Apis.Misc.UI.MsgBoxAsync(I18N.WriteFileFail);
                    break;
            }
        }
