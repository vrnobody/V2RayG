...
namespace ProxySetter.Services
{
    class ServerTracker
    {
        ...
        Apis.Interfaces.Services.IServersService servers;
        Apis.Interfaces.Services.INotifierService notifier;
        ...
        public void UpdateHotkey()
        {
            ...
            if (!Enum.TryParse(bs.hotkeyStr, out Keys hotkey))
            {
                ...
                Apis.Misc.UI.MsgBoxAsync(I18N.ParseKeyCodeFail);
                return;
            }
            ...
            if (string.IsNullOrEmpty(hotKeyHandle))
            {
                ...
                Apis.Misc.UI.MsgBoxAsync(I18N.RegistHotkeyFail);
            }
            else
            {
        ...
        void UpdateSysProxySetting(string servTitle, bool isSocks, int port)
        {
            ...
            var bs = setting.GetBasicSetting();
            bs.proxyPort = Apis.Misc.Utils.Clamp(port, 0, 65536);
        ...
        void OnCoreRunningStatChangeHandler(object sender, EventArgs args)
        {
            if (sender != null)
            {
                var coreCtrl = sender as Apis.Interfaces.ICoreServCtrl;
