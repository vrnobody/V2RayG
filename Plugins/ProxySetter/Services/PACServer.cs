...
namespace ProxySetter.Services
{
    class PacServer
    {
        ...
        {
            var bs = setting.GetBasicSetting();
            var url = Apis.Models.Consts.Webs.FakeRequestUrl;
        ...
        void StartFileWatcher(string relativeFileName)
        {
            ...
            {
                Path = (string.IsNullOrEmpty(path) ? Apis.Misc.Utils.GetAppDir() : path),
        ...
        bool StartListining(string prefix)
        {
            ...
            {
                Apis.Misc.UI.MsgBoxAsync(I18N.StartPacServFail);
                return false;
            }
