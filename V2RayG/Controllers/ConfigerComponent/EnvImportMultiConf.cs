...
namespace V2RayG.Controllers.ConfigerComponet
{
    class EnvImportMultiConf : ConfigerComponentController
    {
        ...
        void BindEvent()
        {
            ...
            {
                var json = Misc.Utils.CreateJObject("v2rayg.configs");
                json["v2rayg"]["configs"][tboxMultiConfPath.Text] = cboxMultiConfAlias.Text;
            ...
            {
                var json = Misc.Utils.CreateJObject("v2rayg.import");
                json["v2rayg"]["import"][tboxImportUrl.Text] = cboxImportAlias.Text;
                container.InjectConfigHelper(() =>
                    Misc.Utils.MergeJson(container.config, json));
            };
            ...
            {
                var json = Misc.Utils.CreateJObject("v2rayg.env");
                json["v2rayg"]["env"][cboxEnvName.Text] = tboxEnvValue.Text;
                container.InjectConfigHelper(
                    () => Misc.Utils.MergeJson(container.config, json));
            };
        }
        ...
        private void InitMultiConfControls()
        {
            ...
            foreach (var item in multiConfItems)
            {
                ...
                var path = Apis.Misc.Utils.RelativePath2FullPath(item.path);
