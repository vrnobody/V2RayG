...
namespace V2RayG.Misc
{
    public static class Utils
    {
        ...
        public static Apis.Models.Datas.StatsSample ParseStatApiResult(string result)
        {
            ...
            foreach (Match match in ms)
            {
                ...
                var value = Apis.Misc.Utils.Str2Int(match.Groups[3].Value);
            ...
            return new Apis.Models.Datas.StatsSample(up, down);
        }
        ...
        public static string GetAppNameAndVer()
        {
            if (string.IsNullOrEmpty(appNameAndVersion))
            {
                ...
                var name = Apis.Misc.Utils.GetAppName();
        ...
        public static List<string> ExtractImportUrlsFrom(JObject config)
        {
            ...
            var import = Misc.Utils.GetKey(config, "v2rayg.import");
        ...
        public static Dictionary<string, string> GetEnvVarsFromConfig(JObject config)
        {
            ...
            var env = GetKey(config, "v2rayg.env");
            if (env == null)
            {
                return empty;
            }
        ...
        {
            var name = GetValue<string>(config, "v2rayg.alias");
            return string.IsNullOrEmpty(name) ? I18N.Empty : name;
        }

        public static string GetSummaryFromConfig(JObject config)
        {
            var strategy = GetValue<string>(config, "routing.balancingRule.0.strategy.type");
            ...
            var result = GetSummaryFromFirstOutbound(config, "outbounds.0");

            return result;
        }

        public static string GetSummaryFromStreamSettings(JObject config, string root)
        {
            var streamType = GetValue<string>(config, root + ".streamSettings.transport")?.ToLower();
            // "tcp" | "kcp" | "ws" | "http" | "quic"
        ...
        public static string GetSummaryFromFirstOutbound(JObject config, string root)
        {
            ...
            string ipKey = root + ".settings.address";

            string ip = GetValue<string>(config, ipKey);
            string streamType = GetSummaryFromStreamSettings(config, root);
            ...
            try
            {
                switch (node)
                {
                    case JObject o:
                        ...
                        return true;
                    case JArray a:
                        a[Apis.Misc.Utils.Str2Int(key)] = new JValue(value);
                        return true;
                    default:
                        break;
                }
        ...
        {
            var result = CreateJObject(@"v2rayg.import");
            foreach (var item in items)
            {
                ...
                {
                    result["v2rayg"]["import"][url] = item.alias ?? string.Empty;
                }
            }
            return result;
        }
        ...
        public static T GetValue<T>(JToken json, string path) =>
            Apis.Misc.Utils.GetValue<T>(json, path);
        ...
        {
            var ip = GetValue<String>(json, prefix, keyIP) ?? Apis.Models.Consts.Webs.LoopBackIP;
        ...
        static public List<string> GetOnlineV2RayCoreVersionList(int proxyPort, string sourceUrl)
        {
            ...
            string pattern = Apis.Models.Consts.Patterns.V2RayCoreReleaseAssets;
            ...
            int proxyPort)
        {
            string[] worker(Models.Datas.SubscriptionItem subItem)
            {
                ...
                var subsString = Fetch(
                    url, proxyPort, Apis.Models.Consts.Import.ParseImportTimeout);
                ...
                var b64s = Apis.Misc.Utils.ExtractBase64Strings(subsString, 24);
        ...
        {
            var url = Apis.Models.Consts.Webs.SearchUrlPrefix + UrlEncode(query);
            if (start > 0)
            {
                url += Apis.Models.Consts.Webs.SearchPagePrefix + start.ToString();
            }
            return url;
        }
        ...
        static bool DownloadFileWorker(string url, string filename, int proxyPort, int timeout)
        {
            ...
            if (timeout <= 0)
            {
                timeout = Apis.Models.Consts.Intervals.DefaultFetchTimeout;
            }

            WebClient wc = new WebClient();
            wc.Headers.Add(Apis.Models.Consts.Webs.UserAgent);

            if (proxyPort > 0 && proxyPort < 65536)
            {
                wc.Proxy = new WebProxy(Apis.Models.Consts.Webs.LoopBackIP, proxyPort);
            }
            ...
            try
            {
                if (!Apis.Misc.Utils.IsHttpLink(url))
                {
                    url = Apis.Misc.Utils.RelativePath2FullPath(url);
                }
        ...
        public static string GetSysAppDataFolder()
        {
            ...
            var appName = Apis.Misc.Utils.GetAppName();
        ...
        public static int Clamp(int value, int min, int max) =>
            Apis.Misc.Utils.Clamp(value, min, max);
            ...
            Apis.Models.Datas.Enums.LinkTypes linkType)
        {
            ...
            switch (linkType)
            {
                case Apis.Models.Datas.Enums.LinkTypes.ss:
                    pattern = GenLinkPrefix(linkType) + "://" + Apis.Models.Consts.Patterns.SsShareLinkContent;
                    break;
                case Apis.Models.Datas.Enums.LinkTypes.vmess:
                case Apis.Models.Datas.Enums.LinkTypes.v2cfg:
                case Apis.Models.Datas.Enums.LinkTypes.v:
                    pattern = GenLinkPrefix(linkType) + "://" + Apis.Models.Consts.Patterns.Base64NonStandard;
                    break;
                case Apis.Models.Datas.Enums.LinkTypes.http:
                case Apis.Models.Datas.Enums.LinkTypes.https:
                    pattern = Apis.Models.Consts.Patterns.HttpUrl;
                    break;
                case Apis.Models.Datas.Enums.LinkTypes.trojan:
                    pattern = GenLinkPrefix(linkType) + "://" + Apis.Models.Consts.Patterns.TrojanUrlContent;
                    break;
                case Apis.Models.Datas.Enums.LinkTypes.vless:
                    pattern = GenLinkPrefix(linkType) + "://" + Apis.Models.Consts.Patterns.UriContent;
                    break;
                default:
                    throw new NotSupportedException($"Not supported link type {linkType.ToString()}:// ...");
            }

            return Apis.Models.Consts.Patterns.NonAlphabets + pattern;
        }
