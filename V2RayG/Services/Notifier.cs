...
namespace V2RayG.Services
{
    public class Notifier :
         ...
         Apis.Interfaces.Services.INotifierService
    {
        Settings setting;
        Servers servers;
        ...
        static readonly long SpeedtestTimeout = Apis.Models.Consts.Core.SpeedtestTimeout;
        static readonly int UpdateInterval = Apis.Models.Consts.Intervals.NotifierMenuUpdateIntreval;

        Bitmap orgIcon;

        Apis.Libs.Tasks.LazyGuy lazyNotifierMenuUpdater;
        ...
        Notifier()
        {
            ...
            ni = new NotifyIcon()
            {
                ...
                Icon = Apis.Misc.UI.GetAppIcon(),
                BalloonTipTitle = Apis.Misc.Utils.GetAppName(),
            ...
            Apis.Libs.Sys.FileLogger.Info($"Create notify icon {handle}");

            CreateContextMenuStrip(niMenuRoot, miServersRoot, miPluginsRoot);

            Apis.Misc.UI.Invoke = Invoke;
            Apis.Misc.UI.InvokeThen = InvokeThen;
        }
            ...
            Updater updater)
        {
            ...
            lazyNotifierMenuUpdater = new Apis.Libs.Tasks.LazyGuy(
        ...
        Apis.WinForms.HotKeyWindow hkWindow = null;

        ConcurrentDictionary<string, Apis.Models.Datas.HotKeyContext> hkContexts =
            new ConcurrentDictionary<string, Apis.Models.Datas.HotKeyContext>();
        ...
        void DestroyHotKeyWindow()
        {
            ...
            Apis.Libs.Sys.FileLogger.Info("Destroy hot key window.");
            var wnd = hkWindow;
            hkWindow = null;
            wnd.ReleaseHandle();
        ...
        void CreateHotKeyWindow()
        {
            ...
            hkWindow = new Apis.WinForms.HotKeyWindow(niMenuRoot);
            var wndHandle = hkWindow.Handle;
            Apis.Libs.Sys.FileLogger.Info($"Create hot key window {wndHandle}.");
            var handles = hkContexts.Keys;
            foreach (var handle in handles)
            {
                if (hkContexts.TryGetValue(handle, out var context))
            ...
            try
            {
                if (hkContexts.TryGetValue(keyMsg, out var context))
                {
                    Apis.Misc.Utils.RunInBackground(() =>
                    {
                        ...
                        {
                            Apis.Libs.Sys.FileLogger.Error($"Handle hotkey error!\n{ex}");
                        }
                    });
                }
            }
            ...
            Action hotKeyHandler, string keyName, bool hasAlt, bool hasCtrl, bool hasShift)
        {
            ...
            var context = Apis.Models.Datas.HotKeyContext.Create(
        ...
        public void ShowFormServerSettings(ICoreServCtrl coreServ)
        {
            if (coreServ == null)
            {
                Apis.Misc.UI.MsgBox(I18N.NullParamError);
                return;
            }
        ...
        public void ScanQrcode()
        {
            ...
            {
                // no comment ^v^
                var video = Apis.Models.Consts.Webs.Nobody3uVideoUrl;
                if (link == video)
                {
                ...
                var msg = Apis.Misc.Utils.AutoEllipsis(link, Apis.Models.Consts.AutoEllipsis.QrcodeTextMaxLength);
        ...
        static void InvokeThenWorker(Control control, Action updater, Action next)
        {
            ...
            Action tail = () =>
            {
                if (next != null)
                {
                    Apis.Misc.Utils.RunInBackground(next);
                }
            };
            ...
            try
            {
                ...
                {
#if DEBUG
                    if (!Apis.Misc.UI.IsInUiThread())
                    {
                        Apis.Libs.Sys.FileLogger.DumpCallStack("!invoke error!");
                    }
#endif
                    worker();
                    tail();
        ...
        private void StartRandomServerInList(List<ICoreServCtrl> list)
        {
            ...
            if (len <= 0)
            {
                Apis.Misc.UI.MsgBoxAsync(I18N.NoServerAvailable);
                return;
            }
        ...
        Apis.Libs.Tasks.Bar serversMenuUpdateBar = new Apis.Libs.Tasks.Bar();
        ...
        void UpdateServersMenuThen()
        {
            ...
            var num = Apis.Models.Consts.Config.QuickSwitchMenuItemNum;
            var groupSize = Apis.Models.Consts.Config.MenuItemGroupSize;

            InvokeThen(() =>
            {
                List<ToolStripMenuItem> miGroupedServers = null;
                if (serverList.Count < Apis.Models.Consts.Config.MinDynamicMenuSize)
                {
                    var miAllServers = ServerList2MenuItems(serverList);
                    miGroupedServers = Apis.Misc.UI.AutoGroupMenuItems(miAllServers, groupSize);
                }
                else
                {
        ...
        void UpdateNotifyIconWorker(Action done)
        {
            ...
            Action finished = () => Apis.Misc.Utils.RunInBackground(() =>
            {
                var relex = UpdateInterval - (DateTime.Now.Millisecond - start);
                Apis.Misc.Utils.Sleep(Math.Max(0, relex));
                done();
            });
            ...
            {
                Apis.Libs.Sys.FileLogger.Error($"Notifier update icon error!\n{e}");
                done();
            }
        }
        ...
        void UpdateNotifyIconTextThen(List<ICoreServCtrl> list, Action finished)
        {
            ...
            void done()
            {
                try
                {
                    ...
                    {
                        int len = Apis.Models.Consts.AutoEllipsis.NotifierSysProxyInfoMaxLength;
                        texts.Add(I18N.CurSysProxy + Apis.Misc.Utils.AutoEllipsis(sysProxyInfo, len));
                    }
        ...
        {
            var maxLen = Apis.Models.Consts.AutoEllipsis.NotifierTextMaxLength;
            var text = string.IsNullOrEmpty(rawText) ?
                I18N.Description :
                Apis.Misc.Utils.AutoEllipsis(rawText, maxLen);
