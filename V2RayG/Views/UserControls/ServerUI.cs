...
namespace V2RayG.Views.UserControls
{
    public partial class ServerUI :
        ...
        Apis.Interfaces.IDropableControl
    {
        Services.Servers servers;
        Services.Settings settings;
        Services.ShareLinkMgr slinkMgr;
        Apis.Interfaces.ICoreServCtrl coreServCtrl;

        string keyword = null;

        Apis.Libs.Tasks.LazyGuy lazyUiUpdater, lazyHighlighter;
        ...
        public ServerUI()
        {
            ...
            lazyUiUpdater = new Apis.Libs.Tasks.LazyGuy(RefreshUiWorker, 400, 3000)
            {
                Name = "ServerUi.RefreshPanel",
            };

            lazyHighlighter = new Apis.Libs.Tasks.LazyGuy(
        ...
        private void ReleaseCoreCtrlEvents(Apis.Interfaces.ICoreServCtrl cs)
        {
            if (cs == null)
            {
                return;
        ...
        private void BindCoreCtrlEvents(Apis.Interfaces.ICoreServCtrl cs)
        {
            if (cs == null)
            {
                return;
        ...
        #region interface Apis.Models.IDropableControl
        ...
        void ShowFormModifyServerSettings() => WinForms.FormModifyServerSettings.ShowForm(coreServCtrl);
        ...
        void HighLightKeyWords()
        {
            ...
            if (string.IsNullOrEmpty(keyword)
                || !Apis.Misc.Utils.PartialMatchCi(title, keyword))
            {
                return;
            }
        ...
        {
            Apis.Misc.UI.Invoke(HighLightKeyWords);
        }
        ...
        void RefreshUiWorker(Action done)
        {
            ...
            Apis.Misc.UI.InvokeThen(worker, next);
        }

        void UpdateNetworkFlowLable(Apis.Interfaces.CoreCtrlComponents.ICoreStates coreState)
        {
            var text = "";
            var tooltip = "";
        ...
        void UpdateInboundModeLabel(Apis.Interfaces.CoreCtrlComponents.ICoreStates coreState)
        {
            var text = @"Config";
            var tooltip = I18N.InbModeConfigToolTip;
            var inbModeIdx = coreState.GetInboundType();
            var lower = coreState.GetInboundIp().Equals(Apis.Models.Consts.Webs.LoopBackIP);

            switch (inbModeIdx)
            {
                case (int)Apis.Models.Datas.Enums.ProxyTypes.HTTP:
                    text = (lower ? @"h" : @"H") + coreState.GetInboundPort();
                    tooltip = @"inbound -> http://" + coreState.GetInboundAddr();
                    break;
                case (int)Apis.Models.Datas.Enums.ProxyTypes.SOCKS:
                    text = (lower ? @"s" : @"S") + coreState.GetInboundPort();
                    tooltip = @"inbound -> socks://" + coreState.GetInboundAddr();
                    break;
                case (int)Apis.Models.Datas.Enums.ProxyTypes.Custom:
                    text = I18N.Custom;
        ...
        private void UpdateTitleTextBox(Apis.Interfaces.CoreCtrlComponents.ICoreStates coreStates)
        {
            var cs = coreStates;
            var summary = Apis.Misc.Utils.AutoEllipsis(cs.GetSummary(), Apis.Models.Consts.AutoEllipsis.ServerSummaryMaxLength);
        ...
        {
            var m = Apis.Misc.Utils.AutoEllipsis(mark, Apis.Models.Consts.AutoEllipsis.MarkLabelTextMaxLength);
        ...
        {
            var m = Apis.Misc.Utils.AutoEllipsis(remark, Apis.Models.Consts.AutoEllipsis.MarkLabelTextMaxLength);
            var tooltip = $"{I18N.Remark}{m}";
            UpdateControlTextAndTooltip(rlbRemark, remark, tooltip);
        }

        void UpdateStatusLable(Apis.Interfaces.CoreCtrlComponents.ICoreStates cs)
        {
        ...
        private void UpdateSettingsLable(Apis.Interfaces.CoreCtrlComponents.ICoreStates coreStates)
        {
        ...
        public void Rebind(Apis.Interfaces.ICoreServCtrl coreServCtrl)
        {
